...
const { Boom } = boom
const { default: akaConnect, useSingleFileAuthState, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, jidDecode, proto } = require("@adiwajshing/baileys")
const {
...
require('./Sky.js')
nocache('../Sky.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./index.js')
...
if (!aka.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(aka, kay, store)
require('./Sky')(aka, m, chatUpdate, store)
} catch (err) {
console.log(err)}})

	// detect group update
		aka.ev.on("groups.update", async (json) => {
			try {
...
aka.sendContact = async (jid, kon, quoted = '', opts = {}) => {
	...
	aka.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
    }
...
else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); akaBot(); }
else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); akaBot(); }
...
aka.sendKatalog = async (jid , title = '' , desc = '', gam , options = {}) =>{
let message = await prepareWAMessageMedia({ image: gam }, { upload: aka.waUploadToServer })
...
return aka.relayMessage(jid, tod.message, {messageId: tod.key.id})
} 

aka.send5ButLoc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
...
aka.relayMessage(jid, template.message, { messageId: template.key.id })
}

aka.sendButImg = async (jid, path, teks, fke, but) => {
...
aka.sendMessage(jid, fjejfjjjer, { quoted: m })
}
